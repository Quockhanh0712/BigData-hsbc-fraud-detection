...
### watch_fraud.ps1 - Live Fraud Alerts
```powershell
.\watch_fraud.ps1
```
Shows new fraud alerts every 3 seconds with:
- Transaction ID (truncated)
- Amount and merchant
- Customer name
- Location (city)

### tail_fraud_logs.ps1 - Detailed Streaming Logs
```powershell
.\tail_fraud_logs.ps1
```
Shows full streaming pipeline logs with fraud details.

## ⚡ Quick Check Script

The `check_system.ps1` script is available:
...
Write-Host "`n[PRODUCER STATUS]" -ForegroundColor Cyan
docker logs producer --tail 3

Write-Host "`n[FRAUD ALERTS]" -ForegroundColor Green
docker exec cassandra cqlsh -e "SELECT COUNT(*) FROM hsbc.fraud_alerts;"

Write-Host "`n[LATEST ALERTS]" -ForegroundColor Yellow
...
Run with: `.\check_system.ps1`

## 🔧 Common Issues

### Issue: "Connection refused" errors
**Fix**: Wait longer for services to start
```powershell
Start-Sleep -Seconds 120
```

### Issue: "Model not found"
**Fix**: Model training not complete, wait or check logs
```powershell
docker logs spark-master | Select-String "AUC-ROC"
```

### Issue: "Out of memory"
**Fix**: Increase Docker memory limit to 8GB+
1. Docker Desktop → Settings → Resources → Memory → 8GB

### Issue: Producer not sending data
**Fix**: Check if df_test_hdfs.csv exists
```powershell
ls data/raw/df_test_hdfs.csv
```
